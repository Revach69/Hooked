...
interface IOSDevice extends DeviceBase {
  screenDensity: number;
}

interface AndroidDevice extends DeviceBase {
  apiLevel: number;
  screenDensity: 'mdpi' | 'hdpi' | 'xhdpi' | 'xxhdpi' | 'xxxhdpi';
  manufacturer: string;
}
...
interface TestDataSet {
  venue_density: Record<string, number>;
  location_scenarios: Record<string, { lat: number; lng: number }>;
...
interface TestMatrixItem {
  ...
  device: string;
  deviceType: 'physical' | 'simulator' | 'emulator';
  testType: string;
...
export const TEST_CONFIGURATIONS: Record<string, TestConfiguration> = {
  'map-interactions': {
    description: 'Core map functionality testing',
    testFiles: ['map-interactions.test.ts'],
    timeout: 30000,
    retry: 2,
  ...
  'location-permissions': {
    ...
    testFiles: ['location-permissions.test.ts'],
    timeout: 20000,
    retry: 3,
  ...
  'performance': {
    ...
    testFiles: ['performance-tests.test.ts'],
    timeout: 60000,
    retry: 1,
  ...
  'small-screen': {
    ...
    testFiles: ['responsive-layout.test.ts'],
    timeout: 15000,
    retry: 2,
  ...
  'large-screen': {
    ...
    testFiles: ['tablet-layout.test.ts'],
    timeout: 20000,
    retry: 2,
    requirements: ['UI Automation']
  },
  'tablet-layout': {
    ...
    testFiles: ['tablet-specific.test.ts'],
    timeout: 25000,
    retry: 2,
  ...
  'samsung-specific': {
    ...
    testFiles: ['samsung-device.test.ts'],
    timeout: 20000,
    retry: 2,
  ...
  'stock-android': {
    ...
    testFiles: ['stock-android.test.ts'],
    timeout: 20000,
    retry: 2,
  ...
  'legacy-support': {
    ...
    testFiles: ['legacy-compatibility.test.ts'],
    timeout: 25000,
    retry: 3,
  ...
  'new-features': {
    ...
    testFiles: ['new-features.test.ts'],
    timeout: 20000,
    retry: 2,
...
export const PLATFORM_SPECIFIC_CONFIGS: Record<string, PlatformConfig & { simulatorConfig?: any; deviceConfig?: any; emulatorConfig?: any; }> = {
  ios: {
    testRunner: 'detox',
 ...
 */
export function generateTestMatrix(): TestMatrixItem[] {
 ...
 */
export function getDeviceConfig(platform: 'ios' | 'android', deviceName: string, deviceType: 'physical' | 'simulator' | 'emulator'): IOSDevice | AndroidDevice | undefined {
  const deviceArray = platform === 'ios' 
    ? (deviceType === 'physical' ? DEVICE_TEST_MATRIX.ios.physical : DEVICE_TEST_MATRIX.ios.simulators)
    : (deviceType === 'physical' ? DEVICE_TEST_MATRIX.android.physical : DEVICE_TEST_MATRIX.android.emulators);
    
  return deviceArray.find(device => device.name === deviceName);
}

/**
 * Validate test requirements for device
 */
export function validateDeviceRequirements(device: IOSDevice | AndroidDevice, testType: string): string[] {
  ...
  requirements.forEach(requirement => {
    switch (requirement) {
      case 'GPS':
        if ('apiLevel' in device && device.apiLevel && !device.model.includes('Pixel')) {
          issues.push('GPS simulation required for emulator');
        }
        break;
      case 'Performance Monitoring':
        if (device.priority !== 'high' && testType === 'performance') {
