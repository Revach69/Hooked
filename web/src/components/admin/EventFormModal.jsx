...
export default function EventFormModal({ event, isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (event) {
        setFormData({
          name: event.name || '',
          event_code: (event.event_code || '').toUpperCase(), // Normalize existing code to uppercase
        ...
        setFormData({
          name: '', event_code: '', location: '', description: '', organizer_email: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // formData.event_code is already uppercased via handleInputChange or useEffect
      const payload = {
        ...formData,
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div>
              ...
              <Input 
                ...
                onChange={(e) => handleInputChange('event_code', e.target.value)} 
                required 
                placeholder="e.g., PARTY2024"
                className={`bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-400 border-gray-300 dark:border-gray-600 ${formErrors.event_code ? 'border-red-500' : ''}`}
              />
              {formErrors.event_code && <p className="text-red-500 dark:text-red-400 text-xs mt-1">{formErrors.event_code}</p>}
            </div>
