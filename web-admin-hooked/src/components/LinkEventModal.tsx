...
interface LinkEventModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onLink: (eventId: string, clientId: string) => Promise<void>;
}

export function LinkEventModal({ event, clients, isOpen, onClose, onLink }: LinkEventModalProps) {
  ...
  const handleLink = async () => {
    ...
    setIsLinking(true);
    try {
      await onLink(event.id, selectedClientId);
      onClose();
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Link Event to Client</DialogTitle>
        </DialogHeader>
